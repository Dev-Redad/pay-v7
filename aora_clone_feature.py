...
def account_cb(update: Update, context: CallbackContext):
    ...
    q.message.reply_text(
        ...
        "✅ You will become <b>admin</b> of the new bot.\n"
        f"👑 Owner stays: <code>{OWNER_ID}</code> (only owner can use /settings).",
        parse_mode=ParseMode.HTML,
        reply_markup=kb
    )
...
def _spawn_clone_bot(token: str, admin_user_id: int, me: dict):
    ...
    env["ADMIN_IDS_CSV"] = str(int(admin_user_id))
...
def account_token_router(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(
        ...
        "👤 You are <b>admin</b> there (not owner).\n"
        f"👑 Owner: <code>{OWNER_ID}</code> (only owner can use /settings)\n\n"
        "⚠️ Important:\n"
        "• Add this new bot as <b>admin</b> in your Storage channel(s), otherwise file uploads won't work.\n"
        f"• If you want it to listen to payment notifications, add it to the payment channel: <code>{PAYMENT_NOTIF_CHANNEL_ID}</code>\n\n"
        f"Log: <code>{html.escape(log_path)}</code>",
        parse_mode=ParseMode.HTML
    )
...
def cmd_start(update: Update, context: CallbackContext):
    ...
    if (uid not in ALL_ADMINS) and CLONE_CREATION_ENABLED:
        ...
        btn_txt = "✅ Create account & start earning" if not acc else ("🤖 Create my bot clone" if not (acc.get("clone_bot") or {}).get("username") else "✅ Bot already created")
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(btn_txt, callback_data="acct:create")]])
        txt = (
            ...
            "You will be <b>admin</b> of that bot (not owner).\n"
            f"Owner stays: <code>{OWNER_ID}</code> (only owner can use /settings)."
            + uid_txt
        )
